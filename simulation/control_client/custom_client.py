...
class HardcodedClient:
    ...
    def lidarSweep (self):
        self.lidarDistances = np.array(self.lidarDistances)
        
        indicesSmallest = np.argpartition(self.lidarDistances, (2, 4))[:4]

        indicesSmallest[indicesSmallest >= self.halfApertureAngle] -= 120

        self.steeringAngle = (
            np.average(indicesSmallest[:2]) * 0.75
            + np.average(indicesSmallest[2:4]) * 0.25
        )
